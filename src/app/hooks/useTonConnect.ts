...
export function useTonConnect() {
  ...
  const { webApp, isReady: isTelegramReady } = useTelegram();
  ...
  useEffect(() => {
    ...
    const initTonConnect = async () => {
      if (typeof window === 'undefined' || !isTelegramReady) return;
      
      try {
        if (!tonConnectUI) {
          // Используем динамический URL для манифеста в зависимости от окружения
  ...
  }, [isTelegramReady, webApp?.colorScheme]);
